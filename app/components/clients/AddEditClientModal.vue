...
/* ================================
  AUTO STATE CODE FROM DROPDOWN
================================ */

watch(
  () => form.value.state,
  (selectedState) => {
    const match = INDIAN_STATES.find(s => s.name === selectedState)
    form.value.stateCode = match?.code || ''
  }
)
...
const submit = async () => {
  if (!form.value.name.trim()) {
    snackbar.value = { show: true, message: 'Client name is required', type: 'error' }
    return
  }
  ...
  try {
    ...
    emit('close')
    emit('notify', {
      message: isEdit.value ? 'Client updated successfully' : 'Client created successfully',
      type: 'success',
    })
...
<template>
  <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div class="bg-white w-full max-w-[600px] rounded-md shadow-lg overflow-hidden flex flex-col"
      ...
      <div class="p-6 space-y-4 text-sm overflow-y-auto">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input v-model="form.code" class="input" placeholder="e.g. CLI001" />
          </div>
          <div>
            <label class="label">Client Name <span class="text-red-600">*</span></label>
        ...
        <div>
          ...
          <input v-model="form.gstNumber" class="input" placeholder="e.g. 24AAAAA0000A1Z5" maxlength="15"
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input v-model="form.pincode" class="input" maxlength="6" />
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <select v-model="form.state" class="input">
              ...
              <option v-for="s in INDIAN_STATES" :key="s.code" :value="s.name">
                {{ s.name }}
              </option>
            </select>
          </div>
          ...
          <div>
            ...
            <input v-model="form.stateCode" class="input bg-gray-100 cursor-not-allowed" disabled />
          </div>
        </div>
      ...
      <div class="px-6 py-4 border-t flex justify-end gap-2 bg-[#FAFBFC]">
        ...
        <button class="btn-primary" :disabled="loading || !form.name.trim()" @click="submit">
